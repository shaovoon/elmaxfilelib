...
#ifdef _MICROSOFT
		pszSplit = wcstok_s( nullptr, m_sDelimit.c_str(), &pszContext );
#else
	#ifdef _WIN32
		pszSplit = wcstok( nullptr, m_sDelimit.c_str() );
	#else
		pszSplit = wcstok( nullptr, m_sDelimit.c_str(), &pszContext );
	#endif
#endif
	}
